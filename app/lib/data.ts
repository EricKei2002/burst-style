...
export const projectsData: Project[] = [
  {
    ...
    title: "学習アプリ",
    description:
      "毎日10問で定着させる学習ドリルPWA。AWS SAA・WordPressなど複数分野に対応し、Googleログイン＋Supabaseで進捗を端末間同期。",
    detailedDescription:
      "「毎日10問」形式の学習PWAです。問題・用語・出題ドメインの配点比率を分野ごとに切り替えられる『学習パック』方式で設計されており、AWS Certified Solutions Architect – AssociateとWordPress運用/開発の2パックに対応、今後も分野を追加できます。各パックの配点比率に沿って出題ドメインを配分し、「未見→誤答→似た論点→古い」の優先順位で決定論的に10問を選出します。解答直後にその場で解説と「持ち帰る一文」を表示し、間違えた論点は弱点データとして記録して弱点ドリル（練習専用画面）で優先的に再出題する設計です（かつてあった復習タブは廃止し、弱点ドリルに導線を一本化）。Googleログイン（Supabase Auth）で連続学習日数・ドメイン別正答率・目標日カウントダウンをアカウントに紐づけて保存し、スマホと自宅PCなど複数端末で同じ記録を引き継げます。学習タブでは略称をタップすると正式名称と意味がその場で開くグロッサリー機能も搭載。ユーザーが自分のGemini APIキーを設定すると弱点分野の問題をその場でAI生成でき（AWSパックでは公式最新情報のWhat's New RSSも加味）、Vercel Cronが毎朝事前生成もしておくため学習時の待ち時間がありません。夜21時にはWeb Push（VAPID）で日課の未消化をリマインドします。問題は公式試験問題の複製ではなく、各分野の公式ドキュメント等を根拠にしたオリジナル演習です。",
    ...
    challenges: [
      ...
      {
        title: "学習パック方式による複数分野対応",
        description:
          "問題・用語・出題ドメインの配点比率をAWS SAA固有の実装から切り離し、『学習パック（StudyPack）』として抽象化しました。抽選ロジックや弱点管理・AI生成エンジンは分野に依存しない共通コアとして実装し、WordPress運用/開発向けパックを追加した際も新しいパック定義を書くだけで済みました。",
      },
    ],
    improvements: [
      {
        title: "学習パックのさらなる拡充",
        description:
          "現在はAWS SAAとWordPressの2パックですが、他の資格・技術分野の学習パックを追加し、対応領域を広げていきたいと考えています。",
      },
      {
        title: "学習分析の強化",
        description:
          "ドメイン別正答率に加えて、間違えた論点同士の関連性を可視化するダッシュボードを検討しています。",
      },
    ],
    documentation: {
